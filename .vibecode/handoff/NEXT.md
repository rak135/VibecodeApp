...
- Dogfood the Phase 1 main TUI shell: run `vibecode` (no args) in a real terminal with `[tui]` extra installed; verify three-column layout renders, R refresh wires correctly, I loads repo map into center panel.
- Phase 2 TUI actions: wire C (context pack), A (agent run), S (session history), G (guard report), T (task input), H (handoff view) keys to real functionality replacing the "not implemented yet" stubs in `vibecode/main_app.py`.
