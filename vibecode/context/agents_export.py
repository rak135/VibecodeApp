...
def render_agents_block() -> str:
    ...
    return (
        ...
        "- `.vibecode/index/*` \u2014 generated index outputs except"
        " `README.md` and `schema.json`\n"
        ...
        "- `vibecode inventory` \u2014 scan files; produce context cards and a risk report"
        " (run before `dashboard` or `serve`)\n"
        ...
        "- `vibecode run` \u2014 explicitly orchestrate an external OpenCode run and then"
        " run guard/check/handoff; use `--guard-mode advisory` (default) or"
        " `--guard-mode strict`\n"
