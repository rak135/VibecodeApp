...
def _missing_textual_message(command: str = "monitor") -> str:
    return (
        f"Error: the 'textual' package is required for 'vibecode {command}'.\n"
        f"Install it with:  pip install 'vibecode[tui]'\n"
...
def cmd_monitor(args: Any) -> int:
    ...
    if not _TEXTUAL_AVAILABLE:
        print(_missing_textual_message("monitor"), file=sys.stderr)
        return 1
